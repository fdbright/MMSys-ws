...
class MonitorSTG:
    ...
    async def check_if_stop(self):
        ...
        for symbol in symbols:
            ...
            if int(self.loop.time() * 1000) - last_update_time > 150000:
                ...
                continue
            order_nums = int(status_dict.get("conf", {}).get("order_nums", 0))
            order_ids = await self.get_order_ids(conn, symbol=symbol)
            if len(order_ids) == 0:
                continue
            else:
                if len(order_ids) < order_nums * 2 - 2:
                    await self.todo.put({"todo": "stop", "symbol": symbol})
                    await self.todo.put({"todo": "start", "symbol": symbol})
                    log.info(f"策略异常-订单数量, 执行重启: {symbol}")
                    continue
        await conn.close()
        del conn
    ...
    def run(self):
        ...
        self.loop.run_sync(self.check_if_stop_by_times)
        self.loop.run_sync(self.check_if_stop_by_order)
        ...
        PeriodicCallback(callback=self.check_if_stop_by_times, callback_time=timedelta(minutes=10), jitter=0.2).start()
        PeriodicCallback(callback=self.check_if_stop_by_order, callback_time=timedelta(minutes=10), jitter=0.2).start()
