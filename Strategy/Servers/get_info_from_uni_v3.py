...
"""
uni_Factory = Web3.toChecksumAddress("0x1F98431c8aD98523631AE4a59f267346ea31F984")
...
class GetInfoFromDEX:
    ...
    def get_weth_price(self):
        weth_usdc_address = self.contract_uni_Factory.caller.getPool(
        ...
        weth_usdc_reserves = weth_usdc_contract.caller.slot0()
        self.weth_price = 1 / (pow(weth_usdc_reserves[0] / pow(2, 96), 2) / pow(10, 18 - 6))
        print(self.weth_price)

    def get_dex_price(self, address: str, _type: str = "weth") -> DexPrice:
        res = DexPrice(type=_type)
        try:
            coin_contract = self.w3.eth.contract(
                address=Web3.toChecksumAddress(self.weth_address),
            ...
            if contract_address != '0x0000000000000000000000000000000000000000':
                ...
                if contract.caller.token0().lower() == address.lower():
                    res.price = (pow(reserves / pow(2, 96), 2) / pow(10, weth_decimal - decimal)) * self.weth_price
                else:
                    res.price = 1 / (pow(reserves / pow(2, 96), 2) / pow(10, weth_decimal - decimal)) * self.weth_price
            else:
                pass
    ...
    def get_price(self):
        for index, symbol in enumerate(self.symbols):
            ...
            dex_price = self.get_dex_price(address=addr, _type="weth")
