...
class Account(MyActionTemplate):
    ...
    async def get(self, item: AccountItem):
        ...
        self.after_request(code=1 if data else -1, msg="获取账户", action=item, data=data)
    ...
    async def post(self, item: AccountItem):
        ...
        if data:
            await self.update2redis(exchange=item.exchange)
        self.after_request(code=1 if data else -1, msg="新增账户", action=item, data=data)
    ...
    async def put(self, item: AccountItem):
        ...
        if data:
            await self.update2redis(exchange=item.exchange)
        self.after_request(code=1 if data else -1, msg="修改账户", action=item, data=data)
    ...
    async def delete(self, item: AccountItem):
        ...
        if data:
            await self.update2redis(exchange=item.exchange)
        self.after_request(code=1 if data else -1, msg="删除账户", action=item, data=data)
